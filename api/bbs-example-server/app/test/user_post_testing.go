...
//
// API "bbs-example-server": user_post TestHelpers
//
// Command:
// $ goagen
...
func CreateUserPostOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.UserPostController, payload *app.UserPostPayload) http.ResponseWriter {
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "UserPostTest"), rw, req, prms)
	createCtx, _err := app.NewCreateUserPostContext(goaCtx, req, service)
	if _err != nil {
...
// IndexUserPostOK runs the method Index of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func IndexUserPostOK(t goatest.TInterface, ctx context.Context, service *goa.Service, ctrl app.UserPostController) (http.ResponseWriter, *app.JSON) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
	...
	goaCtx := goa.NewContext(goa.WithAction(ctx, "UserPostTest"), rw, req, prms)
	indexCtx, _err := app.NewIndexUserPostContext(goaCtx, req, service)
	if _err != nil {
		e, ok := _err.(goa.ServiceError)
		if !ok {
			panic("invalid test data " + _err.Error()) // bug
